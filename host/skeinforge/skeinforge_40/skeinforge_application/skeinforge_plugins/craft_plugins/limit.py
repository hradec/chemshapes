...
class LimitSkein:
	...
	def getZLimitedLineLinear(self, line, location, splitLine):
		...
		# set Z speed to Maximun Z Feed Rate before move X axis up
		if deltaZ>0:
				self.distanceFeedRate.addLine('G1 F%s ; force Z to be maximum Z Feed Rate' % str(self.maximumZFeedRatePerSecond*60.0) )
		return self.getZLimitedLine(deltaZ, distance, line, splitLine)
