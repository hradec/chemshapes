...
try:
	...
	print('You do not have pySerial installed, which is needed to control the serial port.') ; sys.stdout.flush()
	print('Information on pySerial is at:\nhttp://pyserial.wiki.sourceforge.net/pySerial') ; sys.stdout.flush()
...
class RepRapArduinoSerialSender:
	...
	def __init__(self, port, baud, verbose=False):
		...
		if self._verbose:
			print >> sys.stdout, "Opening serial port: " + port ; sys.stdout.flush()

		#Timeout value 10" max travel, 1RPM, 20 threads/in = 200 seconds
		self.ser = serial.Serial(port, baud, timeout=200)

		if self._verbose:
			print >> sys.stdout, "Serial Open?: " + str(self.ser.isOpen()) ; sys.stdout.flush()
			print >> sys.stdout, "Baud Rate: " + str(self.ser.baudrate) ; sys.stdout.flush()
	...
	def reset(self):
		...
		if self._verbose:
			print "Resetting arduino..." ; sys.stdout.flush()
	...
	def close():
		...
		"""
		if self._verbose:
			print >> sys.stdout, "Closing serial port." ; sys.stdout.flush()
		self.ser.close()
